...
/* ---------- workflow timeline (calm, ~8.8s loop) ---------- */

const SEQ = ["idle", "message", "extract", "created", "assigned", "inprogress", "hold"] as const;
...
const FILM_DUR = [1500, 1200, 1600, 999999];
...
function IntakeCard({ reached }: { reached: (k: Step) => boolean }) {
  return (
    <motion.div
      key="intake"
...
export function ProductDashboard({ animated = false, intro = false }: { animated?: boolean; intro?: boolean }) {
  ...
  return (
    <div ref={ref} className="glass edge-light relative w-full overflow-hidden rounded-[22px] border border-white/[0.06] bg-[#0b0b0e]/70">
      ...
      <AnimatePresence>{showCard && <IntakeCard reached={reached} />}</AnimatePresence>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="flex items-center gap-1.5 rounded-full border border-white/[0.07] bg-white/[0.03] px-2.5 py-1.5 backdrop-blur-md">
            <span className="relative flex h-1.5 w-1.5">
              {!reduce && (
                <motion.span
                  className="absolute inset-0 rounded-full bg-[#2E7DFF]"
        ...
        <aside className="hidden w-56 shrink-0 flex-col gap-1 border-r border-white/[0.05] p-3.5 lg:flex">
          ...
          <div className="mt-auto flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-3.5 edge-light">
            <svg width="36" height="36" viewBox="0 0 36 36" className="-rotate-90">
              ...
              <circle cx="18" cy="18" r="15" fill="none" stroke="#2E7DFF" strokeWidth="2.5" strokeLinecap="round" strokeDasharray={`${0.72 * 2 * Math.PI * 15} ${2 * Math.PI * 15}`} />
            </svg>
            <div>
              ...
              <AnimatePresence initial={false}>
                {showTask && (
                  <motion.div
                    key="livetask"
                    layout
                    initial={{ opacity: 0, y: -10, boxShadow: "0 0 0 1px rgba(46,125,255,0.5), 0 0 26px 2px rgba(46,125,255,0.4)" }}
                    animate={{ opacity: 1, y: 0, boxShadow: "0 0 0 1px rgba(46,125,255,0), 0 0 0px 0px rgba(46,125,255,0)" }}
                    exit={{ opacity: 0, height: 0, marginTop: 0, paddingTop: 0, paddingBottom: 0 }}
                    transition={{ opacity: { duration: 0.6, ease }, y: { duration: 0.6, ease }, boxShadow: { duration: 1.1, ease: "easeOut", delay: 0.15 } }}
                    className="grid grid-cols-[1fr_auto] items-center gap-3 rounded-xl border border-[#2E7DFF]/12 bg-[#2E7DFF]/[0.05] px-3 py-3 sm:grid-cols-[1.7fr_1fr_0.9fr]"
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-white/40">
                          ...
                          <AnimatePresence>
                            {reached("assigned") && (
                              <motion.span initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, ease }} className="inline-flex items-center gap-1.5">
                    ...
                    <div className="flex justify-end">
                      <motion.span key={taskLabel} initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease }}>
