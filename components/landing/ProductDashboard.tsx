...
function StatusPill({ tone, children }: { tone: "accent" | "urgent" | "warn" | "muted"; children: React.ReactNode }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium leading-none backdrop-blur-sm ${map[tone]}`}>
      {children}
    </span>
  );
}
...
function NavItem({ icon: Icon, label, active, badge }: { icon: typeof Inbox; label: string; active?: boolean; badge?: number }) {
  return (
    <div
      className={`group relative flex items-center gap-2.5 rounded-xl px-3 py-2 text-[13px] transition-colors duration-300 ${
        active ? "bg-white/[0.07] text-white edge-light" : "text-white/50 hover:bg-white/[0.035] hover:text-white/80"
      }`}
    >
      ...
      {badge ? (
        <span className={`rounded-full px-1.5 py-0.5 text-[10px] leading-none ${active ? "bg-[#2E7DFF] text-white" : "bg-white/[0.06] text-white/55"}`}>
          {badge}
        </span>
      ) : null}
    </div>
...
const stats = [
  { label: "Open requests", to: 14, accent: false },
  { label: "Urgent", to: 2, accent: true },
  { label: "Resolved today", to: 28, accent: false },
  { label: "Arrivals", to: 6, accent: false },
] as const;
...
export function ProductDashboard() {
  return (
    <div className="glass edge-light relative w-full overflow-hidden rounded-[22px] border border-white/[0.06] bg-[#0b0b0e]/70">
      {/* top sheen */}
        ...
        <main className="min-w-0 flex-1 p-5 sm:p-7">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {stats.map((s) => (
              <div
                key={s.label}
                className="lift edge-light relative overflow-hidden rounded-2xl border border-white/[0.05] bg-white/[0.025] p-4 hover:border-white/[0.1] hover:bg-white/[0.04]"
              >
                ...
                <div className={`mt-2.5 text-[32px] font-semibold leading-none tracking-tight tabular-nums ${s.accent ? "text-[#6ba5ff]" : "text-white"}`}>
