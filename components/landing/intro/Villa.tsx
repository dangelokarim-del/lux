...
/**
 * Handcrafted, fully-coded Marbella villa scene — layered light, form and
 * reflection only (no photos, no 3D, no video). Staged by three 0→1 drivers
 * so the hero can tell a story rather than just sit there:
 *
 *   entrance — the dark architectural silhouette fades / rises in.
 *   lights   — warm interior light turns on through the glass bays + pool.
 *   flag     — the master-bedroom window gets a subtle blue "selected" edge.
 *
 * Memoized + driven by quantized numbers, so it re-renders only a handful of
 * times per fade and stays perfectly static during the camera push-in.
 */
export const Villa = memo(function Villa({
  entrance = 1,
  lights = 1,
  flag = 0,
}: {
  ...
  // warm light eases in; afterglow trails it slightly for depth
  const warm = clamp(lights);
  const glow = clamp(lights * 1.05);

  return (
    <div
      className="absolute inset-0 overflow-hidden bg-[#020306]"
      style={{ opacity: lerp(0, 1, entrance), transform: `translateY(${lerp(10, 0, entrance)}px)` }}
    >
      {/* deep dusk sky (always present — this is the silhouette backdrop) */}
      <div
        className="absolute inset-x-0 top-0"
        style={{ height: "63%", background: "linear-gradient(180deg,#03040a 0%,#05070e 42%,#080913 66%,#130e17 88%,#1d1320 100%)" }}
      />
      {/* low horizon afterglow — warms up as the lights come on */}
      <div
        className="absolute inset-x-0"
        style={{
          top: "36%",
          height: "28%",
          background: "radial-gradient(72% 100% at 50% 100%, rgba(255,150,92,0.16), rgba(255,128,80,0.05) 46%, transparent 72%)",
          opacity: lerp(0.35, 1, warm),
        }}
      />
      {[[12, 9], [26, 6], [44, 11], [60, 7], [74, 13], [86, 8], [34, 17], [68, 19], [20, 22]].map(([x, y], n) => (
        <span key={n} className="absolute h-px w-px rounded-full bg-white/35" style={{ left: `${x}%`, top: `${y}%`, opacity: entrance }} />
      ))}
      ...
      <div className="absolute" style={{ left: "72%", right: "16%", top: "44%", height: "6%", background: "linear-gradient(180deg,rgba(255,168,108,0.22),rgba(255,150,92,0.06))", filter: "blur(2px)", transform: "skewX(-12deg)", opacity: warm }} />

      {/* ---- villa massing (the silhouette — dark, always visible) ---- */}
      {/* upper tier */}
      ...
      {/* upper window (decorative) */}
      <div className="absolute" style={{ left: "37%", top: "31%", width: "4%", height: "2.7%", background: "linear-gradient(180deg,rgba(255,190,134,0.4),rgba(255,166,108,0.24))", filter: "blur(0.5px)", opacity: warm }} />

      {/* the master-bedroom "issue" window */}
      <div className="absolute" style={{ left: "56%", top: "30.6%", width: "4.4%", height: "3.4%", opacity: lerp(0.4, 1, warm) }}>
        ...
        <div
          ...
          style={{ background: "radial-gradient(50% 50% at 50% 50%, rgba(255,196,140,0.55), transparent 70%)", filter: "blur(10px)", opacity: lerp(0.25, 1, glow) }}
        />
        {/* subtle blue "selected" edge when the AI targets this window */}
        <div
          className="absolute -inset-[18%] rounded-[2px]"
          style={{
            border: "1px solid rgba(46,125,255,0.65)",
            boxShadow: "0 0 10px rgba(46,125,255,0.35)",
            opacity: clamp(flag) * 0.9,
            transform: `scale(${lerp(1.18, 1, flag)})`,
          }}
        />
      </div>
      ...
      <div className="absolute" style={{ left: "21%", right: "21%", top: "38%", height: "0.45%", background: "rgba(255,170,108,0.45)", filter: "blur(1px)", opacity: warm }} />

      {/* main floor (dark mass) */}
      <div className="absolute" style={{ left: "20%", right: "20%", top: "38.4%", height: "19.6%", background: "linear-gradient(180deg,#080a11 0%,#05060d 100%)" }} />

      {/* ---- everything warm fades up with `lights` ---- */}
      <div className="absolute inset-0" style={{ opacity: warm }}>
        ...
        {/* glass bays — staggered so the lights read as "turning on" */}
        <div className="absolute" style={{ left: "22%", right: "22%", top: "41.4%", height: "13.6%" }}>
          {[0, 1, 2].map((bay) => (
            <div
              key={bay}
              className="absolute top-0 bottom-0 overflow-hidden"
              style={{ left: `${bay * 34}%`, width: "32%", opacity: clamp((lights - bay * 0.16) / 0.5) }}
            >
        ...
        {/* mullions sit above the bays, dark either way */}
        <div className="absolute" style={{ left: "22%", right: "22%", top: "41.4%", height: "13.6%" }}>
          {[32, 66].map((x) => (
      ...
      {/* still water base (dark, present even before lights) */}
      ...
      {/* haze, vignette, leak */}
      <div className="pointer-events-none absolute inset-x-0" style={{ top: "50%", height: "20%", background: "linear-gradient(180deg, rgba(255,168,108,0.045), transparent)", filter: "blur(8px)", opacity: warm }} />
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-1/3" style={{ background: "radial-gradient(60% 100% at 60% 0%, rgba(255,160,104,0.045), transparent 70%)", opacity: warm }} />
    </div>
  );
});
